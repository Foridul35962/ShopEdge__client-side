...
import { verifyResetPassEmail } from '../../store/slices/authSlice'
import { useDispatch, useSelector } from 'react-redux';

const VerifyPasswordOtp = ({ email, setVerified }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await dispatch(verifyResetPassEmail(formData)).unwrap()
            setVerified(true)
        } catch (error) {
                ...
                <div className='flex flex-col sm:flex-row gap-5 sm:gap-10 h-full w-full items-center'>
                    ...
                    <div className='flex flex-col gap-3 w-full sm:w-1/2 '>
                        ...
                        <form onSubmit={handleSubmit} className='flex w-full justify-center  items-center flex-col gap-3'>
                            ...
                            <button className={`bg-blue-800 px-3 sm:w-3/4 py-2 rounded-xl cursor-pointer text-white ${loading ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={loading} type='submit'>Verify & Proccess</button>
                        </form>
                    </div>
                </div>
            </div>
